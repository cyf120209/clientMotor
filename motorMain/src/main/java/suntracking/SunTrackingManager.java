...
//        caculate();
    ...
    private void auto() {
        ...
        DeviceGroup deviceGroup = mSuntrackingPresenter.getDeviceGroup();
        if(lngLat==null || deviceGroup==null){
            return;
        }
...
//            double percent = calClothPercent(0.6, 1.8, 1.2, hs);
            ...
            if(percent1!=lastShadeCmd) {
                ...
                try {
                    Draper.sendCmd(deviceGroup.getDeviceId(), deviceGroup.getGroupId(), 100 + percent1, 7);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
...
//            double rad = calShutter(8, 7, 0, hs, solarAzimuth, pi / 2);
            ...
            if(shutterCmd!=lastShutterCmd){
                ...
                try {
                    Draper.sendCmd(deviceGroup.getDeviceId(),deviceGroup.getGroupId(),shutterCmd,10);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
