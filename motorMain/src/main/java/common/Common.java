package common;

/**
 * Created by lenovo on 2017/2/4.
 */
public class Common {

    public static final int HEIGHT=20;
    public static final int BAUDRATE=38400;

    public static final int SCREEN_WEIGHT=800;
    public static final int SCREEN_HEIGHT=475;


    public static final int DEVICE_FOUND_ALL=1;
    public static final int DEVICE_FOUNDING=0;
    public static final int DEVICE_UPDATE_EXIT=-1;

    public static final int DEVICE_VERSION_SAME=1;
    public static final int DEVICE_VERSION_DEFFER=-1;

//    public static final String STEP_1_START="步骤1开始";
//    public static final String STEP_1_END="步骤1完成";
//    public static final String STEP_2_START="步骤2开始";
//    public static final String STEP_2_END="步骤2完成";
//    public static final String STEP_3_START="步骤3开始";
//    public static final String STEP_3_END="步骤3完成";

    public static final String STEP_1_START="step 1 start";
    public static final String STEP_1_END="step 1 finish";
    public static final String STEP_2_START="step 2 start";
    public static final String STEP_2_END="step 2 finish";
    public static final String STEP_3_START="step 3 start";
    public static final String STEP_3_END="step 3 finish";

    public static final int UPGRADE_12=1;
    public static final int UPGRADE_121=2;

}
